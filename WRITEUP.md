...
A technical writeup, not a research paper. From one project, ended in May 2026, with substantial AI coding assistance throughout.
...
**What came from where.** The project's vision, strategic direction, and ethical frame predate the AI collaboration. The human author built the conceptual foundation before any prompt was sent. That includes the decision to ship on-device, the choice of Apple Intelligence as primary, the willingness to spend twelve weeks on a one-user product, and the design principle this document organizes around (visible > correctable > complete > sophisticated). The principle emerged from the author's judgment during the project, from living through a real trust break, not from an AI suggestion. An AI coding assistant (Anthropic Claude) joined as a collaborator on execution. Claude wrote substantial portions of the code, drafted substantial portions of this text, and helped extract patterns across a large codebase under the author's direction. Where this document says "we did X," it usually means the author decided to do X, Claude implemented or drafted it, both reviewed. The collaboration was real and substantial on both sides. The thinking is the author's. The speed is shared.
...
Entities from different sources (chat-disclosed, contacts-imported, web-scraped) track their source. Same name plus different source equals different rows. The user's Memory view filtered to chat-sourced entities only. Imported data was available to skills but invisible by default.
...
Before overwriting an existing user-self attribute (name, location, work, role, birthplace), require a first-person verb in the originating message. Without this guard, our LLM extraction pipeline produced confident wrong updates from third-person mentions. "My sister lives in Madrid" overwrote the user's location to Madrid.
...
- **Model echoed the user's question back as the answer when uncertain.** Echo guard (token-overlap plus prefix match) at multiple pipeline layers.
...
**First.** The transformer architecture, on which every personal AI product today depends, was given freely to the world. The patterns and lessons that emerge from building on top of it should be given freely too, in the same spirit. We are not contributing comparable scientific weight. We are contributing what one project's experience can contribute. That is a small thing. It is the right thing.
...
To Anthropic, for Claude. A real collaborator on both the code and this writeup. The collaboration model is part of the methodology we disclosed.
...
To the maintainers of `mlx-swift`, `mlx-swift-lm`, `llama.cpp`, `swift-transformers`, and the wider open-source ML community whose work the system depends on.
